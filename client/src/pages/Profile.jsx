  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        ...
        <p className='text-sm self-center'>
          {fileUploadError ? (
            <span className='text-red-700'>
              Error al cargar imagen (La imagen solo debe pesar 2 mb)
            </span>
          ) : filePerc > 0 && filePerc < 100 ? (
            <span className='text-slate-700'>{`Cargando ${filePerc}%`}</span>
          ) : filePerc === 100 ? (
            <span className='text-green-700'>¡Imagen Cargada exitosamente!</span>
          ) : (
            ''
          )}
        </p>
        ...
        <input
          ...
          onChange={handleChange}
        />
        <button disabled={loading} className="bg-red-700 text-white p-3 rounded-lg uppercase hover:opacity-95 dissabled:opacity-80">
    ...
    <Link to={'/create-listing'} className="bg-green-700 text-white p-3 rounded-lg uppercase text-center hover:opacity-95">
      ...
      <div className="flex justify-between mt-5">
        <span onClick={handleDeleteUser} className="text-red-700 cursor-pointer">Borrar cuenta</span>
        <span onClick={handleSignOut} className="text-red-700 cursor-pointer">Cerrar sesión</span>
      </div>
      <p className="text-red-700 mt-5">{error ? error: ''}</p>
      <p className="text-green-700 mt-5">{updateSuccess ? '¡Cuenta actualizada exitosamente!' : ''}</p>
      {currentUser && currentUser.isAdmin === true && (
       <div>
         <button onClick={handleShowListings} className="text-green-700 w-full">Ver mis anuncios</button>
        <p className="text-red-700 mt-5">{showListingsError ? 'Error al cargar anuncios' : ''}</p>
       </div>
      )}
      ...
      { userListings
       ...
       <div className="flex flex-col gap-4">
        ...
        <h1 className="text-center mt-7 text-2xl font-semibold">Sus Anuncios</h1>
